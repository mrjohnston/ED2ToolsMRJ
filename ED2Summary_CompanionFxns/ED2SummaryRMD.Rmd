...
###See ED/src/init/ed_params for PFT parameter values, but note that these are often overwritten by XML.
```{r, echo=FALSE,comment=NA}
pfts<-paste("PFTs: ",ed2in[grepl("^[[:blank:]]*[^[:blank:]!]",ed2in)&grepl("NL%INCLUDE_THESE_PFT",ed2in)])
pfts_num<-as.numeric(Numextract(pfts))
cat(paste("PFTs in model: ",paste(pfts_num,collapse=",")))
#source("/home/miriam/git/ED2ToolsMRJ/ED2Summary_CompanionFxns/Block4_ExtractFromedparams.R")
```
...
if("E"%in%tscales){
  ...
  if(length(subtimeslistyrs)==24){
    plot(subtimeslistyrs[1:12],mmean_carbon_ac[1:12],type="o",pch=20,xlab="month",ylab="MMEAN_CARBON_AC_PY [umol/m2/s]",
    ...
    plot(subtimeslistyrs,mmean_carbon_ac,pch=20,xlab="month",ylab="MMEAN_CARBON_AC_PY [umol/m2/s]",
         ylim=c(min(mmean_carbon_ac),max(mmean_carbon_ac)),col=ycols)
